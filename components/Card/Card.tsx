...
const Card = ({ images, src, hasTransparentBg, alt, id, setModalOpen }: { images: string[]; src: string; hasTransparentBg?: boolean; alt: string; id: number; setModalOpen: React.Dispatch<React.SetStateAction<number>> }) => {
  ...
  return (
    <div onClick={OpenId} className={`flex-1 cursor-pointer rounded-md relative ${hasTransparentBg ? "bg-white" : ""} ${!hasSrc ? "justify-center flex items-center" : ""} aspect-square`}>
      <div className="z-[59] text-white absolute top-1 right-1 px-2 rounded-md bg-black bg-opacity-50">{len !== 1 ? `1 / ${len}` : ""}</div>
      {hasSrc ? (
        <Image src={src} layout="fill" objectFit="contain" alt={alt} className="w-full h-full rounded-md select-none" onContextMenu={(e) => e.preventDefault()} />
      ) : (
